...
describe("gamma encoding", () => {
  it("should encode choices to integers", () => {
    ...
    let encoded = encoding.encode([1, 0], 2);
    let decoded = encoding.decode(encoded, 2);
    expect(arithm.toNumber(encoded)).toEqual(4);
    expect(decoded).toEqual([1,0]);
    encoded = encoding.encode([150, 10, 125], 300);
    decoded = encoding.decode(encoded, 300);
    expect(arithm.toNumber(encoded)).toEqual(13458406);
    expect(decoded).toEqual([150, 10, 125]);
    ...
    decoded = encoding.decode(encoded, 0)
    expect(arithm.toNumber(encoded)).toEqual(0);
    expect(decoded).toEqual([]);

    // test cached
    encoded = encoding.encode([1, 0], 2);
    decoded = encoding.decode(encoded, 2);
